...
def check_volatility(vix_data):
    """
    변동성 억제 확인
    VKOSPI <= 20 또는 5일 전보다 하락
    """
    current_vix = vix_data.get('current', 25)
    vix_5d_ago = vix_data.get('5d_ago', 25)
...
def check_theme_persistence(theme_data):
    """
    테마 지속성 확인
    동일 테마 3일 연속, 상승 종목 >= 2개
    """
    themes_lasting = []
    
    for theme in theme_data:
        if (theme.get('consecutive_days', 0) >= 3 and 
            theme.get('advancing_stocks', 0) >= 2):
            themes_lasting.append(theme['name'])
    
    return len(themes_lasting) > 0
...
def determine_risk_regime(market_data, vix_data, theme_data):
    ...
    return {
        ...
        "score": score,
        "max_score": 3,
        "playbook": playbook_map.get(score, "관망"),
        "factors": {
            "breadth": breadth_ok,
            "volatility": volatility_ok,
            "theme": theme_ok
        },
        "note": "Risk_ON = 사도 죽지 않을 확률이 높다 (매수 신호 아님)"
    }
